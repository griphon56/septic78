<?php
  ...
  /**
   * Метод получения продуктов хитов продаж Септиков.
   *
   * @param array|null $a_product_category <tt>Массив ключей категорий товара</tt>, для вывода всех товаров одной категории.
   * @param bool $is_not <tt>Если <b>true</b> - тогда метод вернёт товары которые НЕ относятся к категориям из массива.</tt>
   * @return array <tt>Массив продуктов.</tt>
   */
  function getProductListHitSales(array $a_product_category=null, bool $is_not=false)
  {
    $link = mysqli_connect(HOST, USER, PASS,DB) or die('No connect to Server');
    mysqli_set_charset($link,'utf8');
    ...
    $query_a = "
      ...
      from
        ...
        product_category on
          product_category.k_product_category=product.k_product_category
      where
        product.k_product in(30, 147, 75, 162) and
        ".$where."
      order by
        product.s_name,
          product.i_price
    ...
    $r_query_a = mysqli_query($link,$query_a);

    $a_product_a = [];
    while($row = mysqli_fetch_assoc($r_query_a)){
      $a_product_a[] = $row;
    }
    ...
    $r_query_b = mysqli_query($link,$query_b);
    mysqli_close($link);

    $a_product_b = [];
    while($row = mysqli_fetch_assoc($r_query_b)){
      $a_product_b[] = $row;
    }

    return array_merge($a_product_a,$a_product_b);
  }
?>